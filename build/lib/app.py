...
def query(host, record_type):
    if record_type not in {'A','AAAA','MX','TXT','CAA'}:
    ...
    with open_no_redirect(req,timeout=6) as response:
        payload=json.loads(bounded_read(response).decode('utf-8'))
    if not isinstance(payload,dict): raise ValueError('DNS response was not an object')
    answers=payload.get('Answer',[])
    if answers is None: return []
    if not isinstance(answers,list): raise ValueError('DNS Answer field was not a list')
    values=[]
    for answer in answers[:64]:
        if isinstance(answer,dict) and isinstance(answer.get('data'),str): values.append(answer['data'][:2048])
    return values
